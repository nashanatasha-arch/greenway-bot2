...
TEAM_URL = os.getenv("TEAM_URL")  # ссылка на команду / чат / анкету, если понадобится
...
async def show_career_flow(update: Update):
    await update.message.reply_text(
        "🚀 Если ты ищешь не подработку, а основной источник дохода — важно сразу идти по системе.\n\n"
        "Здесь не про «попробовать».\n"
        "Здесь про:\n"
        "• выйти на стабильный доход\n"
        "• перестать зависеть от найма\n"
        "• построить систему, а не разовые продажи\n\n"
        "Я задам тебе несколько вопросов — это поможет понять, как быстро ты можешь выйти на результат."
    )
...
async def ask_income_goal(update: Update):
    ...
    await update.message.reply_text(
        "Теперь про цифры 👇\n\n"
        "Какой доход ты хочешь начать получать в ближайшие 2–3 месяца?\n\n"
        "Важно ответить честно — от этого зависит стратегия.",
        reply_markup=ReplyKeyboardMarkup(keyboard, resize_keyboard=True),
    )
...
async def ask_time_commitment(update: Update):
    ...
    await update.message.reply_text(
        ...
        "Это напрямую влияет на скорость результата.",
        reply_markup=ReplyKeyboardMarkup(keyboard, resize_keyboard=True),
    )
...
async def ask_career_reason(update: Update):
    await update.message.reply_text(
        "Скажи честно 👇\n\n"
        "Почему ты сейчас рассматриваешь смену формата дохода?\n\n"
        "Например:\n"
        "• устал(а) от работы по найму\n"
        "• не устраивает доход\n"
        "• хочу больше свободы\n"
...
async def handle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user.get("stage") == "collect_career_deadline":
        ...
        await update.message.reply_text(
            "Смотри 👇\n\n"
            f"Твоя ситуация:\n"
            f"• причина: {user['career_reason']}\n"
            f"• цель: {user['goal']}\n"
            f"• доход: {user['income_goal']}\n"
            f"• время: {user['time_commitment']}\n"
            f"• срок: {user['career_deadline']}\n\n"
            "Это уже не уровень «посмотреть».\n"
            "Это заявка на результат.\n\n"
            "Если ты реально готов(а) действовать — я покажу тебе простой старт без перегруза.\n\n"
            "Напиши: СТАРТ"
        )
        await send_lead_card(context, user_id)
        return
